...
int getImg(int img) { myprintf("enter getImg");
	if (sprites[img].hole) return 9;
	return img;
}
int getAndDecImg(int img) { myprintf("enter getAndDecImg");
...
int getLength(int num) { myprintf("enter getLength");
...
void showMemoryError() { myprintf("enter showMemoryError");
	clearClosedSet(0);
	clearClosedSet(1);
	clearOpenSet(0);
	clearClosedSet(1);
	while (aptMainLoop()) {
		myprintf("!.rError: ran out of memory...");
		keepConsole();
		hidScanInput();
...
void addClosedSet(int x, int y, int s) { myprintf("enter addClosedSet");
	Path * newNode;
...
void clearClosedSet(int s) { myprintf("enter clearClosedSet");
...
void remClosedSet(int x, int y, int s) { myprintf("enter remClosedSet");
	if (closedSetN[s] <= 0) return;
    Path * c;
    Path * prev;
    c = closedSet[s];
...
bool openSetNotEmpty(int s) { myprintf("enter openSetNotEmpty");
	return openSetN[s] != 0;
}
int sanitizeX(int x) { myprintf("enter sanitizeX");
	if (x < 2) return 396;
	if (x > 396) return 2;
	return x;
	/*if (x > 398) return 0;
...
int sanitizeY(int y) { myprintf("enter sanitizeY");
	if (y < 2) return 236;
	if (y > 236) return 2;
	return y;
	/*if (y > 238) return 0;
...
bool inClosedSet(int x, int y, int s) { myprintf("enter inClosedSet");
	Path * c = closedSet[s];
    while (c) {
...
bool inOpenSet(int x, int y, int s) { myprintf("enter inOpenSet");
	Path * c = openSet[s];
    while (c) {
    	if (c->x == x && c->y == y) return true;
		c = c->next;
...
void addOpenSet(int x, int y, int s) { snprintf(mystring,sizeof(mystring),"enter addOpenSet %d %d %d",openSetN[s],closedSetN[s],totalPathN[s]); myprintf(mystring);
	Path * newNode;
	newNode = malloc(sizeof(Path));
	if (!newNode) {
		showMemoryError();
...
void clearOpenSet(int s) { myprintf("enter clearOpenSet");
...
void remOpenSet(int x, int y, int s) { myprintf("enter remOpenSet");
	if (openSetN[s] <= 0) return;
    Path * c;
    Path * prev = NULL;
    c = openSet[s];
...
int getDist(int x, int y, int dx, int dy) { myprintf("enter getDist");
	if (!options[0]) {
		/*if (dx % 2 != x % 2) {
...
void addNode(int x, int y, int nx, int ny, int myGScore, int s) { myprintf("enter addNode");
	int ac = getBot(s);
	gscore[s][nx][ny] = myGScore;
	fscore[s][nx][ny] = myGScore + getDist(nx,ny,sprites[ac].x >> 8,sprites[ac].y >> 8) * epsilon;
}
void addNodeToApple(int x, int y, int nx, int ny, int myGScore, int s) { myprintf("enter addNodeToApple");
	cameFrom[s][nx][ny].x = x;
	cameFrom[s][nx][ny].y = y;
	gscore[s][nx][ny] = myGScore;
	fscore[s][nx][ny] = myGScore + getDist(nx,ny,apple.x >> 8,apple.y >> 8) * epsilon;
}
void continueAddNodeToApple(int x, int y, int nx, int ny, int myGScore, int s) { myprintf("enter continueAddNodeToApple");
	cameFrom[s][nx][ny].x = x;
	cameFrom[s][nx][ny].y = y;
	gscore[s][nx][ny] = myGScore;
	fscore[s][nx][ny] = myGScore + getDist(nx,ny,apple.x >> 8,apple.y >> 8) * epsilon;
...
int getDirectionXTo(int i, int x, int s) { myprintf("enter getDirectionXTo");
...
int getDirection(int s) { myprintf("enter getDirection");
	int ac = getBot(s);
	int r = 0;
...
int getDirectionToApple(int s) { myprintf("enter getDirectionToApple");
	int r = 0;
	int smallest = INT_MAX;
	for (int i = 0; i < 4; i++) {
...
void setCurrentF(int s) { myprintf("enter setCurrentF");
	int smallest = INT_MAX;
	Path * c = openSet[s];
	while (c) {
...
bool haveDirection() { myprintf("enter haveDirection");
	for (int i = 0; i < 4; i++) {
		if (directions[i]) return true;
	}
	return false;
}
static int UDSSend(Message msg) { myprintf("enter UDSSend");
...
int getMoveableRange(int s) { myprintf("enter getMoveableRange");
...
void getMoveableRangeApple(int s) { myprintf("enter getMoveableRangeApple");
...
bool noCameFrom(int x, int y, int s) { myprintf("enter noCameFrom");
	return (!cameFrom[s][x][y].x && !cameFrom[s][x][y].y);
}
bool isCameFrom(int x, int y, int s) { myprintf("enter isCameFrom");
	return (cameFrom[s][x][y].x || cameFrom[s][x][y].y);
}
void showCameFrom(int s) { myprintf("enter showCameFrom");
...
void getFarthestCurrent(int s) { myprintf("enter getFarthestCurrent");
	int ac = getBot(s);
	Path *c = closedSet[s];
	int pdis = 0;
...
void plotMovement(int s) { myprintf("enter plotMovement");
	int ac = getBot(s);
...
/*void setCloseCurrent() { myprintf("enter setCloseCurrent");
	int i = 0;
...
bool pathfindToApple(int s) { myprintf("enter pathfindToApple");
...
void relocateApple() { myprintf("enter relocateApple");
	apple.x = ((rand() % (400 - 64)) + 30) << 8;
	apple.y = ((rand() % (240 - 64)) + 30) << 8;
	while (getColor(apple.x >> 8, apple.y >> 8) || getColor((apple.x >> 8) + 1, (apple.y >> 8) + 1)) {
		apple.x = ((rand() % (400 - 64)) + 30) << 8;
...
static void changeApple() { myprintf("enter changeApple");
...
static void moveApple() { myprintf("enter moveApple");
	int oldx = apple.x;
	int oldy = apple.y;
	relocateApple();
...
static void updateApple(int x, int y) { myprintf("enter updateApple");
	int oldx = apple.x;
	int oldy = apple.y;
	apple.x = x;
	apple.y = y;
...
static void setApple(int player, int x, int y) { myprintf("enter setApple");
...
bool checkPath(int s) { myprintf("enter checkPath");
...
void showPlotMovement(int s) { myprintf("enter showPlotMovement");
...
void showPathToApple(int s) { myprintf("enter showPathToApple");
	giveUpTimer[s] = (u64)0;
	plotting[s] = false;
	int ac = getBot(s);
	if (ac >= numPlayers()) {
...
int livingBots() { myprintf("enter livingBots");
	int r = 0;
...
void continuePlotting(int s) { myprintf("enter continuePlotting");
	...
	cameFrom[x][y].y = 1;*/
	int i = 0;
	u64 thinkingTime = 13 * TICKS_PER_MS;
...
void plotCourse(int s) { myprintf("enter plotCourse");
	int ac = getBot(s);
	if (sprites[ac].dead) return;
	plotting[s] = false;
	clearClosedSet(s);
...
static int lagMult() { myprintf("enter lagMult");
	if (num_bikes <= 4) return 1;
	int r = (num_bikes - 4) / 2;
	return r > 0 ? r : 1;
}
static void UDSResend(bool replied[], Message msg) { myprintf("enter UDSResend");
...
static bool allReplied(bool replied[]) { myprintf("enter allReplied");
	for (int i = 0; i < num_bikes; i++) {
		if (!replied[i] && i != myNum) return false;
	}
	return true;
}
static int UDSDirect(int node, Message msg) { myprintf("enter UDSDirect");
	msg.sender = myNum;
	ret=0;
	if(conntype!=UDSCONTYPE_Spectator)//Spectators aren't allowed to send data.
	{
...
bool itsATie() { myprintf("enter itsATie");
	int s = score[0];
	int winner = 0;
	for (int i = 0; i < numPlayers(); i++) {
...
int getHighestScore() { myprintf("enter getHighestScore");
	int highestScore = 0;
	if (itsATie()) return -1;
	int r = 0;
	for (int i = 0; i < numPlayers(); i++) {
...
int theHighest() { myprintf("enter theHighest");
	int r = 0;
	for (int i = 0; i < numPlayers(); i++) {
...
bool everyoneElseIsDead(int n) { myprintf("enter everyoneElseIsDead");
	for (int i = 0; i < numPlayers(); i++) {
		if (i != n && !sprites[i].dead) return false;
	}
	return true;
}
bool allReady() { myprintf("enter allReady");
	for (int i = 0; i < num_bikes; i++) {
		if (!ready[i]) return false;
	}
	return true;
}
int notReadies() { myprintf("enter notReadies");
	int r = 0;
	for (int i = 0; i < num_bikes; i++) {
		if (!ready[i]) r++;
	}
...
int getKiller(u32 color) { myprintf("enter getKiller");
...
bool hasCommonY(int num) { myprintf("enter hasCommonY");
...
int getAlive() { myprintf("enter getAlive");
	int k = 0;
...
static void setSprites() { myprintf("enter setSprites");
	for (int b = 0; b < numBots; b++) {
		usedSpecial[b] = false;
		giveUpTimer[b] = (u64)0;
	}
...
{ myprintf("enter print_constatus");
	ret=0;
	u32 pos;
...
u64 debugUDSSent;
//---------------------------------------------------------------------------------
static void sceneInit(void) { myprintf("enter sceneInit");
//---------------------------------------------------------------------------------
	for (int i = 0; i < 30; i++) {
...
static void reversePath(int n) { myprintf("enter reversePath");
	Path temp;
	int start = pathPos[n];
...
static void printScore() { myprintf("enter printScore");
	if (!options[7]) {
...
static void drawLine(int pathnum,u32 sx, u32 sy, u32 dx, u32 dy, int img) { myprintf("enter drawLine");
	int w = (dx >> 8) - (sx >> 8);
	int h = (dy >> 8) - (sy >> 8);
	int x = sx;
	int y = sy;
...
static void fixLength(int img) { myprintf("enter fixLength");
...
static void finishLine(int pathnum,u32 sx, u32 sy, u32 dx, u32 dy, Sprite msg, int img) { myprintf("enter finishLine");
...
static void eraseOvershoot(Sprite sprite) { myprintf("enter eraseOvershoot");
...
static void gameOptions() { myprintf("enter gameOptions");
	myconsoleClear();
	char onOrOff[5];
...
//---------------------------------------------------------------------------------
static void moveSprites() { myprintf("enter moveSprites");
//---------------------------------------------------------------------------------
	int i;
...
//---------------------------------------------------------------------------------
static void sceneRender(void) { myprintf("enter sceneRender");
//---------------------------------------------------------------------------------
	int i;
	// Update the uniforms
	C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, uLoc_projection, &projection);
...
//---------------------------------------------------------------------------------
static void sceneExit(void) { myprintf("enter sceneExit");
//---------------------------------------------------------------------------------
...
void setHighscore(u32 score) { myprintf("enter setHighscore");
	highScore = score;
...
//---------------------------------------------------------------------------------
int main(int argc, char **argv) { myprintf("enter main");
//---------------------------------------------------------------------------------
	srand(time(NULL));
	// Initialize graphics
	gfxInitDefault();
